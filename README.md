...
This repository contains my first attempt to complete the [iCapital Identity Team Engineer Challenge](https://gist.github.com/pm-builder/979dcf6300d4dff1cdfd2ee4808af091), which is supposed to be finished in two hours. During this attempt, I did **not** employ the help of any kind of artificial intelligence.
...
Shortly after getting to the end of the non-AI version, I realized that I hadn't actually accomplished very much, as described more below. I decided to go ahead and try to build the same project using ChatGPT as an aide, while also working with the two hour time limit. That version, if you're interested, is [located here](https://github.com/natedugg/icapital-with-ai).
...
Unfortunately, the most important feature didn't make it in, which was a working form for saving investor profile data and a way to upload related files. The investor list index page should work in isolation, but that's about it. A form without the file upload was almost done but still broken when time ran out.

Beyond the MVP, there are many other additions and enhancements that I would have included given more time.

- Authentication / authorization: I would have added JWT / session token-based authentication to limit functionality for non-administrative users. This would probably require a separate `users` table and setup of administrative roles for admin users.
- Full implementation of the file upload feature, which means storing file metadata in the database and file contents on the file system.
  ...
  - A much better, more scalable solution would be to use cloud storage, like AWS S3, which uses pre-signed URLs for upload/download and multi-part upload for large files. Here are the high-level implementation details for that:
    - When uploading a file, first the file metadata is uploaded to the API, which saves the metadata to the database
    - If it's a very large file, the multi-part calculation is done on the API and an array of pre-signed URLs for each upload part is returned to the client
    ...
    - Multi-part uploads bring a more benefits
      - supports resumable uploads by keeping track of chunks already uploaded, and then simply resuming with the last chunk that has not yet completed
      - supports progress bars on the front end with the ability to keep track of successfully uploaded chunks
    - File downloads become a two step process
      ...
      - the client can then temporarily, securely, and directly access the full file using the pre-signed download URL
...
- **END-TO-END INTEGRATION TESTS**
  - For file uploads, I think this would be important to verify that this feature is working
